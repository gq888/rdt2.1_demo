...
"""
RDT2.1协议10%丢包率环境测试
通过修改sender.py的模拟丢包功能来测试协议在高丢包环境下的表现
"""
...
def test_with_simulated_packet_loss():
    ...
    if "# 丢包模拟" not in content:
        # 找到_send_and_wait方法，添加丢包模拟
        modified_content = content.replace(
            "def _send_and_wait(self, pkt: Packet, expect_type: int, expect_ack: Optional[int] = None) -> Packet:",
            '''def _send_and_wait(self, pkt: Packet, expect_type: int, expect_ack: Optional[int] = None) -> Packet:
        """发送并等待响应，包含详细的丢包重传日志"""
        # 丢包模拟：10%概率模拟发送超时（仅在测试环境下）
        import random
        if hasattr(self, '_test_packet_loss_rate') and random.random() < self._test_packet_loss_rate:
    ...
    # 创建一个临时脚本来启用丢包模拟
    test_sender_script = TEST_DIR / "test_sender_with_loss.py"
    with open(test_sender_script, 'w') as f:
        f.write(f'''
import sys
...
sock.settimeout(0.3)
cfg = SenderConfig()
sender = RdtSender(sock, ("127.0.0.1", 6666), cfg)
...
''')
    ...
    result = subprocess.run([sys.executable, str(test_sender_script)], 
    ...
    if test_sender_script.exists():
        test_sender_script.unlink()
...
if __name__ == "__main__":
    test_with_simulated_packet_loss()