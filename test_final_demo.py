...
DOWNLOADS_DIR = TEST_DIR / "test_downloads_final_demo"
...
def send_file_with_detailed_logging(file_path: Path, host: str, port: int, scenario_name: str, **kwargs) -> dict:
    ...
    print(f"    📦 丢包事件: {packet_loss_events}")
    print(f"    ⏱️  RTO更新: {rto_updates}")
    ...
    return {
        ...
        "stats": {
            ...
            "rto_updates": rto_updates
        }
    }

def demo_simple_comparison():
    """演示简单对比 - 正常网络 vs 轻丢包网络"""
    print("=" * 80)
    print("RDT2.1 丢包重传行为详细分析演示")
    print("对比正常网络 vs 轻丢包网络的传输行为")
    print("=" * 80)
    
    # 创建测试文件
    test_file = create_test_file("packet_loss_demo.txt", 50)  # 50KB
    ...
    try:
        ...
        if received_file.exists():
            ...
            print(f"\n[正常网络] 文件完整性验证: {'✅ 通过' if results['normal']['file_integrity'] else '❌ 失败'}")
        else:
    ...
    # 测试2: 轻丢包网络 (1% 丢包) - 使用较低丢包率避免握手问题
    print(f"\n{'='*80}")
    print("[测试2] 轻丢包网络环境 (1% 丢包率)")
    print(f"{'='*80}")
    ...
    try:
        ...
        if received_file.exists():
            ...
            print(f"\n[轻丢包网络] 文件完整性验证: {'✅ 通过' if results['light_loss']['file_integrity'] else '❌ 失败'}")
        else:
...
def main():
    ...
    try:
        success = demo_simple_comparison()
        if success:
